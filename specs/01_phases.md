...
| --------- | ----------------------------- | ----------------------------------------------- |
| Runtime   | Bun                           | Fast, TypeScript-native, per project guidelines |
| Framework | React 18+                     | Familiar, component model fits UI well          |
| Language  | TypeScript                    | Type safety, per requirements                   |
| Styling   | Tailwind CSS                  | Utility-first, rapid iteration                  |
| Storage   | localStorage                  | Simple, no backend needed                       |
| Icons     | macosicons.com (pre-fetched)  | High-quality macOS-style icons                  |
| Build     | Bun.serve() with HTML imports | Per project guidelines, no Vite                 |
...
1. **Get macosicons.com API key** (user action)
2. **Phase 1 implementation** - Set up project, build base keyboard UI
3. **Write icon fetch script** (can run once API key is available)

---

## Open Questions

1. **Layer switching UX**: Hold-to-view vs toggle? Need to test what feels natural.
2. **Key sizing**: How large do keys need to be for app names? May need responsive breakpoints.
3. **Edit workflow**: Click-to-edit vs dedicated edit mode? TBD in Phase 6.
